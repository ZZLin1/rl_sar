...
public:
    ...
    void Run() override
    {
        ...
        {
            fsm_command->motor_command.q[i] = 0;
            fsm_command->motor_command.dq[i] = 0;
            fsm_command->motor_command.kp[i] = 5;
            fsm_command->motor_command.kd[i] = 0.5;
...
class HEX4FSMFactory : public FSMFactory
{
public:
    HEX4FSMFactory(const std::string& initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<hex4_fsm::RLFSMStatePassive>(rl);
        else if (state_name == "RLFSMStateGetUp")
            return std::make_shared<hex4_fsm::RLFSMStateGetUp>(rl);
        else if (state_name == "RLFSMStateGetDown")
            return std::make_shared<hex4_fsm::RLFSMStateGetDown>(rl);
        else if (state_name == "RLFSMStateRLLocomotion")
            return std::make_shared<hex4_fsm::RLFSMStateRLLocomotion>(rl);
        return nullptr;
    }
    std::string GetType() const override { return "hex4"; }
...
REGISTER_FSM_FACTORY(HEX4FSMFactory, "RLFSMStatePassive")
